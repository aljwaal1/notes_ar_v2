...
class Note {
  ...
  static Note decode(String raw) {
    final p = raw.split('|||');
    return Note(p.isNotEmpty ? p[0] : 'ملاحظة', p.length > 1 ? p[1] : '', p.length > 2 ? p[2] : 'عام', p.length > 3 ? p[3] == '1' : false, p.length > 4 ? DateTime.tryParse(p[4]) ?? DateTime.now() : DateTime.now());
  }
...
class _HomeState extends State<Home> {
  ...
  Future<void> load() async {
    ...
    final saved = p.getStringList('notes_v3') ?? p.getStringList('notes_v2');
    setState(() { notes = (saved == null || saved.isEmpty) ? starter() : saved.map(Note.decode).toList(); reminders = p.getBool('notes_reminders') ?? true; reminderTime = TimeOfDay(hour: p.getInt('notes_h') ?? 21, minute: p.getInt('notes_m') ?? 0); });
  }
  ...
  Future<void> save() async { final p = await SharedPreferences.getInstance(); await p.setStringList('notes_v3', notes.map((e) => e.encode()).toList()); await p.setBool('notes_reminders', reminders); await p.setInt('notes_h', reminderTime.hour); await p.setInt('notes_m', reminderTime.minute); }

  List<Note> get visible {
    final q = searchCtrl.text.trim();
    return notes.where((n) {
      final f = filter == 'الكل' || (filter == 'المفضلة' && n.fav) || n.tag == filter;
  ...
  void addNote() {
    ...
    setState(() { notes.insert(0, Note(title.isEmpty ? 'ملاحظة بدون عنوان' : title, body, tag, false, DateTime.now())); titleCtrl.clear(); bodyCtrl.clear(); });
    save();
